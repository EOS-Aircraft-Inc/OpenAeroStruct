"""Wing 6 planform rendering -- the wing 5 2D/3D set, redrawn for wing 6.

Wing 6 is wing 5 with the twist required to fall monotonically from the root to
the winglet junction. The reference outline is the ConstChord as-built wing, same
convention as the wing 5 figure. Wing 5's free twist is drawn in the twist panel
only -- that panel is where the two designs differ at all; everywhere else the
monotonicity constraint moves the planform by too little to see, which is itself
the result.

Wing 6 is rebuilt from the stored design vector in
``out/logs/wing6_design_point.json`` and the as-built reference from
...
BASE_C, C5, C6 = "#4C72B0", "#c0392b", "#1f6f4a"
BOX_C = "#55A868"
AIL_C = "#8172B2"
CROSS_C = "#2e8b57"
...
def figure_2d(base, w6, xs, ret, stick, ref, stick_base, free):
    global W5_FREE_DRAG
    W5_FREE_DRAG = float(free["drag_N"])
    fig = plt.figure(figsize=(16, 12))
    fig.suptitle(
        "WING 6 — wing 5 with monotonically decreasing twist, root → tip\n"
        f"drag {base['drag']:.0f} → {w6['drag']:.0f} N ({w6['drag'] / base['drag'] - 1:+.2%}) at MTOW, "
        f"S_ref {base['S_ref']:.2f} → {w6['S_ref']:.2f} m², span pinned at 118 ft — "
        f"monotonicity costs {w6['drag'] - W5_FREE_DRAG:+.0f} N ({w6['drag'] / W5_FREE_DRAG - 1:+.3%}) against wing 5",
        fontsize=13,
    )
    ...
    y, le, te = planform(w6["mesh"])
    ax.plot(y, le, color=C5, lw=1.6, label="wing 6")
    ax.plot(y, te, color=C5, lw=1.6)

    y, front, rear = box_edges(w6["mesh"])
    ...
    ax.plot(w6["y_toc"], w6["toc"], color=C5, lw=2.2,
            label=f"wing 6 (root {w6['log']['toc_root']:.3f}, wing 5's t/c)")
    t5_on_r = np.interp(ref[0], w6["y_toc"], w6["toc"])
    ...
    ax.set_title("Thickness — wing 5's profile, carried over unchanged", fontsize=11)
    ...
    y5, d5 = depth_along_span(w6, xs, ret, stick, ref)
    ax.plot(yb, db, color=BASE_C, lw=1.8, ls="--", label="as-built depth at aft spar")
    ax.plot(y5, d5, color=C5, lw=2.2, label="wing 6 depth at aft spar")
    ...
    for w, c, lab, ls in ((base, BASE_C, "as-built", "--"), (w6, C5, "wing 6", "-")):
        y, le, te = planform(w["mesh"])
    ...
    for w, c, lab, ls in ((base, BASE_C, "as-built", "--"), (w6, C5, "wing 6", "-")):
        ...
        ax.plot(y, w["twist"], color=c, lw=1.8 if lab != "wing 6" else 2.2, ls=ls, label=lab)
    ax.plot(free["y_in"], free["twist_abs"], color=C6, lw=1.6, ls=":",
            label="wing 5 (free twist)")
    ax.axvline(Y_JUNCTION, color="0.55", lw=1.2)
    ax.annotate("constraint ends\nat the junction", xy=(Y_JUNCTION, ax.get_ylim()[1]),
                xytext=(-6, -4), textcoords="offset points", ha="right", va="top",
                fontsize=8, color="0.35")
    ax.axvline(Y_AIL, color=AIL_C, lw=1.5, ls="--")
    ax.set_xlabel("y, in"); ax.set_ylabel("twist, deg")
    ax.set_title("Twist — monotonic root → junction, the point of wing 6", fontsize=11)
    ax.grid(alpha=0.3); ax.legend(fontsize=9)

    path = os.path.join(OUT_DIR, "wing6_planform.png")
    fig.savefig(path, dpi=130, bbox_inches="tight")
    plt.close(fig)
    return path


def figure_3d(base, w6):
    fig = plt.figure(figsize=(16, 7))
    fig.suptitle("WING 6 — isometric, true scale (dihedral to scale)", fontsize=13)

    for i, (w, c, lab) in enumerate(((base, BASE_C, "ConstChord as-built"), (w6, C5, "wing 6"))):
        ...
        ax.plot(fp[thick, 1], fp[thick, 0], fp[thick, 2], color=CROSS_C, lw=3.0,
                label="thickened region" if lab == "wing 6" else "as-built t/c")
    ...
    path = os.path.join(OUT_DIR, "wing6_3d.png")
    fig.savefig(path, dpi=130, bbox_inches="tight")
    plt.close(fig)
    return path
...
if __name__ == "__main__":
    ...
    with open(os.path.join(LOGS, "wing6_design_point.json")) as fh:
        dp = json.load(fh)
    ...
    w6 = rebuild(dp["wing6_mtow"], W.REGION_A_END_IN)
    free = dp["wing5_mtow"]
    xs, ret = retention_fn()
    print(f"  as-built {base['drag']:9.1f} N   wing 6 {w6['drag']:9.1f} N "
          f"({w6['drag'] / base['drag'] - 1:+.2%})")
    print(f"  S_ref    {base['S_ref']:.3f} -> {w6['S_ref']:.3f} m²")
    ...
    print(f"  depth at the aileron: {np.interp(Y_AIL, *depth_along_span(w6, xs, ret, stick, ref)):.2f} in "
          f"against {DEPTH_REQ:.0f} in required")

    print(f"  wing 5 free twist {free['drag_N']:9.1f} N -> wing 6 {w6['drag']:9.1f} N "
          f"({w6['drag'] / free['drag_N'] - 1:+.3%} for monotonicity)")

    for p in (figure_2d(base, w6, xs, ret, stick, ref, stick_base, free), figure_3d(base, w6)):
        print(f"  wrote {p}")
